-- Standalone SQL Script to Create Project Backlog Tables
-- Run this script manually if Flyway is not available or if you need to run it separately
-- Usage: psql -h host -p port -U username -d database -f run-backlog-migration.sql
...
CREATE INDEX IF NOT EXISTS idx_backlog_stories_project ON backlog_stories(project_id);
CREATE INDEX IF NOT EXISTS idx_backlog_stories_original_story ON backlog_stories(original_story_id);
CREATE INDEX IF NOT EXISTS idx_backlog_stories_original_sprint ON backlog_stories(original_sprint_id);
CREATE INDEX IF NOT EXISTS idx_backlog_stories_status ON backlog_stories(status);
CREATE INDEX IF NOT EXISTS idx_backlog_stories_created_from_sprint ON backlog_stories(created_from_sprint_id);

-- Backlog Tasks indexes
CREATE INDEX IF NOT EXISTS idx_backlog_tasks_story ON backlog_tasks(backlog_story_id);
CREATE INDEX IF NOT EXISTS idx_backlog_tasks_original_task ON backlog_tasks(original_task_id);
CREATE INDEX IF NOT EXISTS idx_backlog_tasks_status ON backlog_tasks(status);
CREATE INDEX IF NOT EXISTS idx_backlog_tasks_overdue ON backlog_tasks(is_overdue);

-- Backlog Subtasks indexes
CREATE INDEX IF NOT EXISTS idx_backlog_subtasks_task ON backlog_subtasks(backlog_task_id);
CREATE INDEX IF NOT EXISTS idx_backlog_subtasks_original_subtask ON backlog_subtasks(original_subtask_id);
CREATE INDEX IF NOT EXISTS idx_backlog_subtasks_completed ON backlog_subtasks(is_completed);

-- Success message
DO $$
BEGIN
    RAISE NOTICE 'Project backlog tables created successfully!';
END $$;
