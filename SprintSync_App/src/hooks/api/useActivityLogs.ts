...
export const useProjectActivities = (projectId: string, days: number = 30) => {
  ...
  const fetchProjectActivities = useCallback(async () => {
    ...
    try {
      ...
      console.log(`Fetching project activities for projectId="${projectId}"`);
      // Use the new dedicated endpoint for project activities
      const response = await activityLogApiService.getProjectActivities(projectId, days);
      console.log('Project activities API response:', response);

      const logs = parseActivityLogResponse(response);
      console.log(`Parsed ${logs.length} project activity logs`);
  ...
  return { activityLogs, loading, error, refetch: fetchProjectActivities };
};
